Shader "hda/Exam/ShieldURP"
{
    ...
    CBUFFER_START(UnityPerMaterial)
		...
		float4 _UVTiling;
		float _Activation;
		float4 _ActivationDir;
		float _ActivationSmooth;
		...
		float _WobbleSpeed;
		float _WobbleSpeedVar;
		float _WobbleStrength;
		float _WobbleStrengthVar;

		float4 _WaveColor;
		float _WaveHitStrength;
		float _WaveMaxTime;
		float _WaveDelay;
		float4 _WaveCenter;
		float _WaveThickness;
		float _FadeDelay;
		float4 _WaveHitList[32];
    CBUFFER_END
	...
	Varyings vert(Attributes IN) //Vertex Shader
	{
		...
		output.uv = IN.uv;
		output.positionWS = TransformObjectToWorld(IN.pos);
		output.positionCS = TransformObjectToHClip(IN.pos);
		output.normalWS = TransformWorldToObjectNormal(IN.normal);
	...
	{
		//View Direction
		float3 viewDir = normalize(IN.positionWS.xyz - _WorldSpaceCameraPos);

		//Fresnel Factor, clamped to 0 - 1
		float fresnel = saturate(1 - dot(viewDir, normalize(IN.normalWS)));
		
		//Get info about uv & hex;
		float2 uv = IN.uv.xy * _UVTiling.xy + _UVTiling.zw;
		float2 unscaledUV = IN.uv.xy * 2 - 1;
    ...
    SubShader //multiple subshaders for different GPUs, Unity will choose the most suited one for current application
    {
        ...
        Pass //BASE PASS, FRONT FACE
        {
			...
			//------------------------------------------------------------------
			ENDHLSL //here ends the pure Cg code
        }
    }
}
