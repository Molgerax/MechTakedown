    ...
    SubShader 
    {
        ...
        Pass 
        {
			...
			float2 ParallaxOcclusionMapping(float2 uv, float3 viewDirTS)
            {
				...
				float2 currentUV = uv;
            	float currentDepthMap = SAMPLE_TEXTURE2D_LOD(_ParallaxMap, sampler_ParallaxMap, TRANSFORM_TEX(currentUV, _BumpMap), 0).r;
            	...
            	while(currentLayerDepth < currentDepthMap && i < numLayers)
            	{
            		...
            		currentDepthMap = SAMPLE_TEXTURE2D_LOD(_ParallaxMap, sampler_ParallaxMap, TRANSFORM_TEX(currentUV, _BumpMap), 0).r;
				...
				float surfaceOffsetBeforeDepth = SAMPLE_TEXTURE2D_LOD(_ParallaxMap, sampler_ParallaxMap, TRANSFORM_TEX(previousUV, _BumpMap), 0).r 
            										- currentLayerDepth + layerDepth;
            ...
            float ParallaxSelfShadowing(float2 uv, float3 lightDirTS)
			{
			    ...
			    float2 currentUV = uv;
			    float currentDepthMapValue = SAMPLE_TEXTURE2D(_ParallaxMap, sampler_ParallaxMap, TRANSFORM_TEX(uv, _BumpMap)).r;
			    ...
			    {
			        currentUV += stepVector;
			        currentDepthMapValue = SAMPLE_TEXTURE2D(_ParallaxMap, sampler_ParallaxMap, TRANSFORM_TEX(currentUV, _BumpMap)).r;
			        currentLayerDepth -= layerDepth;
			        iterationCount++;
			    }
